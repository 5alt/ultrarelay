...
class HTTPRelayServer(Thread):
    ...
    class HTTPHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
        ...
        def do_ntlm_negotiate(self, token, proxy):
            ...
            #print self.challengeMessage.getData().encode('hex')
            #print hex(self.challengeMessage['flags'])
            try:
               self.challengeMessage = self.challengeMessage.getData()[:20]+'\x05\x02\x8a\xa2'+self.challengeMessage.getData()[24:]
               #self.challengeMessage = self.challengeMessage.getData()[:20]+'\x05\x82\x8a\xa2'+self.challengeMessage.getData()[24:]
            except Exception,e:
               print e
            challenge.fromString(self.challengeMessage)
